...
# ✅ Port range untuk run project (dari 9001-9100)
PORT_START          = 9001
PORT_END            = 9100
...
# ✅ Global storage untuk running processes
running_processes: dict = {}   # {project_id: {"process": Popen, "port": int, "logs": [], "started_at": ts}}
...
def bersihkan_code(text: str) -> str:
    """Bersihkan markdown fence dari code"""
    if not text:
        return text
    text  = text.strip()
    lines = text.split("\n")
...
def buat_project_background(project_id: str, deskripsi: str, nama: str):
    try:
        ...
        system_plan = (
            ...
            f'  "files": ["main.py","requirements.txt","README.md","tests/test_main.py"],\n'
        ...
        # ✅ Auto-add pytest & fastapi jika perlu
...
# ================================================
# ✅ RUN PROJECT — API BARU
# Jalankan project di background dengan port unik
# ================================================
...
def stop_project(project_id: str) -> bool:
    """Stop running process untuk project"""
    with _proc_lock:
...
def run_project(project_id: str, project_dir: Path, run_cmd: str, project_type: str) -> dict:
    """Jalankan project di background"""
    # Stop dulu jika sudah running
    stop_project(project_id)

    port = cari_port_kosong()

    # Modifikasi run_cmd untuk web framework agar pakai port yang benar
    env = os.environ.copy()
    env["PORT"] = str(port)

    if project_type == "fastapi":
        # Coba deteksi & override
        if "uvicorn" not in run_cmd:
            # Cari file main.py atau app.py
            for candidate in ["main:app", "app:app"]:
                mod_name = candidate.split(":")[0]
                if (project_dir / f"{mod_name}.py").exists():
                    run_cmd = f"uvicorn {candidate} --host 0.0.0.0 --port {port}"
                    break
        else:
            ...
            if "--host" not in run_cmd:
                run_cmd = run_cmd + f" --host 0.0.0.0"
    ...
    try:
        ...
        # Tunggu sedikit untuk cek apakah langsung crash
        time.sleep(2)
        ...
        return {
            ...
            "run_cmd"  : run_cmd,
            "type"     : project_type,
            "url"      : f"http://localhost:{port}" if project_type in ["fastapi", "flask"] else None,
        }
    ...
    with _proc_lock:
        ...
        if info:
            ...
            data["running"] = {
                "alive"     : is_alive,
                "port"      : info["port"],
                "url"       : f"http://localhost:{info['port']}" if info.get("type") in ["fastapi", "flask"] else None,
...
async def run_logs_route(project_id: str):
    """Ambil live logs dari running process"""
    with _proc_lock:
        info = running_processes.get(project_id)
        if not info:
        ...
        return JSONResponse({
            "logs"       : info["logs"][-200:],  # 200 baris terakhir
            "alive"      : is_alive,
            "port"       : info["port"],
            "url"        : f"http://localhost:{info['port']}" if info.get("type") in ["fastapi", "flask"] else None,
            "exit_code"  : exit_code,
...
async def running_list_route():
    """List semua project yang sedang running"""
    hasil = []
    with _proc_lock:
